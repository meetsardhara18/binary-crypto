...
function configurable(opt) {
  throw new Error(`${name}: ${opt} is not a configurable property`);
}
...
function optType(opt, type) {
  throw new Error(`${name}: ${opt} option must have a value of type ${type}`);
}
 ...
 */
function optsType() {
  throw new Error(`${name}: opts param must be an object`);
}

function negative(opt) {
  throw new Error(`${name}: ${opt} must be greater than or equal to 0`);
}
...
function spinnersType() {
  throw new Error(`${name}: spinners param is not an object or array`);
}
...
function spinnersEmpty() {
  throw new Error(`${name}: object or array of spinners is empty`);
}
...
function invalidState(state) {
  throw new Error(`${name}: ${state} is not a valid spinner state`);
}
